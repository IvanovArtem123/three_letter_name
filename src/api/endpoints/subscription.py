...
from api.exceptions import forbidden, not_found
...
from api.keys import AddUserToInbounds, ManageUserUI
...
async def create_subscription(
    ...
    await user_crud.update(user_in_sub, user_update, session)  # Делаем пользователя не новым
...
async def get_keys_by_sub_code(
    ...
    if await check_headers(request=request):
        ...
        keys_str = '\n'.join(keys)
        encoded_keys = base64.b64encode(keys_str.encode('utf-8')).decode('utf-8')
    ...
    return PlainTextResponse(
        ...
        headers={
            ...
            'announce': f'base64:{base64.b64encode(ANNOUNCE_HAPP.encode("utf-8")).decode("utf-8")}',
